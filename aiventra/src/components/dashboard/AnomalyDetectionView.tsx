"use client";
/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unused-vars, react/jsx-no-comment-textnodes */

import React, { useState, useEffect, useMemo } from "react";
import { motion } from "framer-motion";
import {
  Activity, AlertTriangle, TrendingUp, ShieldAlert, Radio, Zap,
  Smartphone, Users, DollarSign, MapPin, Loader2
} from "lucide-react";
import { getAnomalyFromCase } from "@/lib/api";
import { EmptyState } from "./EmptyState";
import type { AnomalyReport, AnomalyFinding } from "@/lib/api";

// ── Derive chart data from real anomalies ────────────────────────────────────
function buildStreamData(anomalies: AnomalyFinding[]) {
  return Array.from({ length: 60 }, (_, i) => {
    const anomalyInfluence = anomalies.reduce((sum, a) => {
      const dist = Math.abs(i - (a.threat_score % 60));
      return sum + (a.threat_score * Math.exp(-dist * 0.05));
    }, 0);
    return {
      x: i,
      current: 40 + Math.sin(i * 0.3) * 15 + anomalyInfluence * 0.3 + (Math.random() - 0.5) * 10,
      baseline: 30 + Math.sin(i * 0.25) * 20,
    };
  });
}

function buildBaselineData(deviationScore: number) {
  return Array.from({ length: 40 }, (_, i) => ({
    x: i,
    current: 40 + Math.sin(i * 0.3) * 30 + (deviationScore * 0.3) * Math.sin(i * 0.5),
    baseline: 30 + Math.sin(i * 0.25) * 20,
  }));
}

const REASON_ICONS = [TrendingUp, Radio, Smartphone, DollarSign, Users, MapPin, Activity, Zap];

export default function AnomalyDetectionView() {
  const [report, setReport] = useState<AnomalyReport | null>(null);
  const [loading, setLoading] = useState(true);
  const [time, setTime] = useState("");

  useEffect(() => {
    const tick = () => setTime(new Date().toLocaleTimeString("en-US", { hour12: false }));
    tick();
    const iv = setInterval(tick, 1000);
    return () => clearInterval(iv);
  }, []);

  useEffect(() => {
    setLoading(true);
    getAnomalyFromCase("AIV-2041-77")
      .then((r) => { setReport(r.data); })
      .catch(() => setReport(null))
      .finally(() => setLoading(false));
  }, []);

  const threatScore = report ? Math.round(report.overall_threat_score) : 0;
  const anomalies = report?.anomalies ?? [];
  const streamData = useMemo(() => anomalies.length ? buildStreamData(anomalies) : [], [anomalies]);
  const baselineData = useMemo(() => report ? buildBaselineData(report.behavioral_profile.deviation_score) : [], [report]);
  const escalation = report ? Math.round(report.escalation_probability) : 0;
  const factors = anomalies.flatMap(a => a.contributing_factors);

  // Derive projection stats from escalation probability
  const projections = report ? [
    { label: "Next 5 MIN", value: `${Math.min(100, Math.round(escalation * 0.82))}%` },
    { label: "Next 15 MIN", value: `${Math.min(100, Math.round(escalation * 0.93))}%` },
    { label: "Next 30 MIN", value: `${Math.min(100, escalation)}%` },
    { label: "Next 60 MIN", value: `${Math.min(100, Math.round(escalation * 1.05))}%` },
  ] : [];

  const threatColor = threatScore >= 80 ? "text-red-500" : threatScore >= 50 ? "text-amber-500" : "text-teal-400";
  const threatBg = threatScore >= 80 ? "bg-red-500" : threatScore >= 50 ? "bg-amber-500" : "bg-teal-500";

  return (
    <div className="flex flex-col h-full bg-[#05070b] text-slate-200 font-sans overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-black/20">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 text-crimson font-bold">
            <ShieldAlert size={20} />
            <span className="font-orbitron text-lg tracking-[0.15em] uppercase">Anomaly Detection</span>
          </div>
          <div className="h-4 w-px bg-white/10" />
          <div className="font-mono text-[10px] text-slate-500 uppercase tracking-widest">
            // Behavioral Threat Analysis Engine
          </div>
        </div>
        <div className="flex items-center gap-8">
          <div className="text-right">
            <div className="text-[10px] font-mono text-slate-500 uppercase tracking-tighter">
              Case: <span className="text-teal-400">{report?.case_id ?? "—"} ●</span>
            </div>
            <div className="text-[10px] font-mono text-slate-500 uppercase tracking-tighter">
              Threat: <span className={threatColor}>{report?.overall_threat_level ?? "PENDING"}</span>
            </div>
          </div>
          <div className="text-right">
            <div className="text-sm font-mono text-slate-300">{time}</div>
            <div className="text-[10px] font-mono text-slate-500 uppercase">
              {new Date().toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" }).toUpperCase()}
            </div>
          </div>
        </div>
      </div>

      {/* Body */}
      <div className="flex-1 p-6 relative">
        {loading && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <Loader2 size={32} className="animate-spin text-crimson" />
          </div>
        )}
        {!loading && !report && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-12">
            <EmptyState message="No behavioral anomalies detected. Upload evidence files to trigger neural pattern analysis." />
          </div>
        )}
        <div className={`grid grid-cols-[1fr_300px_320px] gap-6 h-full overflow-y-auto custom-scrollbar ${!report ? 'opacity-20 pointer-events-none grayscale' : ''}`}>

          {/* ─── Left Column ─────────────────────────────────────── */}
          <div className="flex flex-col gap-6">
            {/* Live Anomaly Stream */}
            <div className="bg-black/40 rounded-xl border border-white/5 p-5 relative">
              <div className="flex items-center justify-between mb-6">
                <div className="flex flex-col">
                  <div className="flex items-center gap-2 font-orbitron text-xs text-slate-300 uppercase tracking-widest">
                    <div className={`w-1.5 h-1.5 rounded-full ${report ? 'bg-crimson animate-pulse' : 'bg-slate-700'}`} />
                    Live Anomaly Stream
                  </div>
                  <div className="text-[9px] font-mono text-slate-500 mt-1 uppercase">
                    {report ? `${anomalies.length} anomalies detected · Threat Score: ${threatScore}` : "Waiting for evidence ingestion"}
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-[9px] font-mono text-slate-500 uppercase">
                    Stream: <span className={report ? "text-teal-400" : "text-slate-600"}>{report ? "LIVE" : "IDLE"}</span>
                  </div>
                </div>
              </div>

              <div className="h-64 relative w-full">
                <div className="absolute left-0 top-0 bottom-0 w-8 flex flex-col justify-between text-[8px] font-mono text-slate-600 border-r border-white/5 pr-2">
                  <span>100</span><span>75</span><span>50</span><span>25</span><span>0</span>
                </div>
                <svg className="absolute left-8 right-0 top-0 bottom-0 w-[calc(100%-32px)] h-full overflow-visible">
                  {[25, 50, 75].map(y => (
                    <line key={y} x1="0" y1={`${y}%`} x2="100%" y2={`${y}%`} stroke="rgba(255,255,255,0.05)" strokeDasharray="4 4" />
                  ))}
                  {streamData.length > 0 && (
                    <>
                      <path
                        d={`M ${streamData.map(d => `${d.x * 12},${128 - (d.baseline * 1.2)}`).join(' L ')}`}
                        fill="none" stroke="#14B8A6" strokeWidth="1" strokeOpacity="0.4"
                      />
                      <path
                        d={`M ${streamData.map(d => `${d.x * 12},${128 - (d.current * 1.2)}`).join(' L ')}`}
                        fill="none" stroke="#F8FAFC" strokeWidth="1.5"
                      />
                      {/* Anomaly burst markers at positions derived from real anomaly scores */}
                      {anomalies.map((a, i) => {
                        const xPos = (a.threat_score / 100) * 700 + 20;
                        const yPos = 128 - (a.threat_score * 1.2);
                        return (
                          <g key={a.anomaly_id} transform={`translate(${xPos}, ${Math.max(10, yPos)})`}>
                            <circle r="3" fill="#ef4444" className="animate-pulse" />
                            <text x="0" y="-8" textAnchor="middle" fontSize="6" className="font-orbitron fill-red-500 uppercase">{a.severity}</text>
                          </g>
                        );
                      })}
                    </>
                  )}
                </svg>
              </div>
            </div>

            {/* Behavioral Drift Wave */}
            <div className="bg-black/40 rounded-xl border border-white/5 p-5 flex-1 min-h-[160px] relative">
              <div className="font-orbitron text-[10px] text-slate-400 uppercase tracking-widest mb-4">Behavioral Drift Wave</div>
              <div className="absolute inset-x-5 bottom-10 top-12">
                <svg className="w-full h-full" preserveAspectRatio="none">
                  <defs>
                    <linearGradient id="driftGradient" x1="0" y1="0" x2="1" y2="0">
                      <stop offset="0%" stopColor="#14B8A6" stopOpacity="0.8" />
                      <stop offset={`${report ? Math.max(20, 100 - report.behavioral_profile.deviation_score) : 50}%`} stopColor="#14B8A6" stopOpacity="0.2" />
                      <stop offset={`${report ? Math.min(80, 100 - report.behavioral_profile.deviation_score + 20) : 50}%`} stopColor="#C0182A" stopOpacity="0.2" />
                      <stop offset="100%" stopColor="#C0182A" stopOpacity="0.8" />
                    </linearGradient>
                  </defs>
                  <path
                    d={`M 0 40 ${Array.from({ length: 20 }).map((_, i) => {
                      const amp = report ? 10 + (report.behavioral_profile.deviation_score * 0.4) : 30;
                      return `Q ${i * 40 + 20} ${i % 2 === 0 ? 40 - amp : 40 + amp}, ${i * 40 + 40} 40`;
                    }).join(' ')}`}
                    stroke="url(#driftGradient)" strokeWidth="2" fill="none"
                  />
                </svg>
              </div>
              <div className="absolute bottom-5 left-5 right-5 flex justify-between font-orbitron text-[8px] tracking-widest uppercase">
                <span className="text-teal-400">Stability</span>
                <span className="text-crimson">Instability</span>
              </div>
            </div>

            {/* Anomaly Event Feed — purely from report.anomalies */}
            <div className="bg-black/40 rounded-xl border border-white/5 p-5">
              <div className="flex items-center gap-2 font-orbitron text-[10px] text-slate-300 uppercase tracking-widest mb-4">
                <div className={`w-1.5 h-1.5 rounded-full ${anomalies.length ? 'bg-crimson' : 'bg-slate-700'}`} />
                Anomaly Event Feed
              </div>
              <div className="space-y-2">
                {anomalies.length === 0 && (
                  <div className="text-[10px] font-mono text-slate-600 uppercase italic py-4 text-center">No anomalies recorded</div>
                )}
                {anomalies.slice(0, 5).map((a) => (
                  <FeedItem
                    key={a.anomaly_id}
                    time={a.detected_at}
                    label={a.anomaly_type.replace(/_/g, " ")}
                    sub={a.description}
                    severity={a.severity === "CRITICAL" ? "SEVERE" : a.severity}
                  />
                ))}
              </div>
              {anomalies.length > 5 && (
                <div className="text-center mt-3 text-[9px] font-mono text-slate-500">
                  +{anomalies.length - 5} more anomalies
                </div>
              )}
            </div>
          </div>

          {/* ─── Middle Column ────────────────────────────────────── */}
          <div className="flex flex-col gap-6">
            {/* Threat Escalation Meter */}
            <div className="bg-black/40 rounded-xl border border-white/5 p-5 flex flex-col items-center flex-1">
              <div className="font-orbitron text-[10px] text-slate-400 uppercase tracking-widest mb-8">Threat Escalation Meter</div>
              <div className="relative w-16 flex-1 max-h-[400px] mb-8">
                <div className="absolute inset-0 rounded-full border border-white/10 bg-gradient-to-b from-white/5 to-transparent flex flex-col justify-end p-1">
                  <motion.div
                    className="w-full rounded-full bg-gradient-to-t from-green-500 via-yellow-500 via-orange-500 to-red-500"
                    initial={{ height: 0 }}
                    animate={{ height: `${threatScore}%` }}
                    transition={{ duration: 1.5, ease: "easeOut" }}
                  >
                    <div className="w-full h-full animate-pulse bg-white/20" />
                  </motion.div>
                </div>
                <div className="absolute left-full ml-4 top-0 bottom-0 flex flex-col justify-between font-mono text-[9px] text-slate-500 py-2">
                  <div className="flex items-center gap-2"><span className="text-crimson">Critical</span><span>{'>'} 90</span></div>
                  <div className="flex items-center gap-2"><span className="text-red-500">Severe</span><span>70 - 90</span></div>
                  <div className="flex items-center gap-2"><span className="text-amber-500">Moderate</span><span>40 - 70</span></div>
                  <div className="flex items-center gap-2"><span className="text-yellow-500">Low</span><span>10 - 40</span></div>
                  <div className="flex items-center gap-2"><span className="text-teal-400">Minimal</span><span>{'<'} 10</span></div>
                </div>
              </div>
              <div className="text-center">
                <motion.div
                  className={`text-4xl font-mono font-bold ${threatColor} bg-red-500/10 px-4 py-2 rounded-lg border border-red-500/20`}
                  initial={{ scale: 0.5, opacity: 0 }}
                  animate={{ scale: 1, opacity: 1 }}
                  transition={{ duration: 0.6, delay: 0.3 }}
                >
                  {threatScore}
                </motion.div>
                <div className="mt-2 font-orbitron text-[10px] text-slate-500 uppercase tracking-widest">Threat Score</div>
              </div>
            </div>

            {/* Predictive Threat Projection */}
            <div className="bg-black/40 rounded-xl border border-white/5 p-5 h-[340px]">
              <div className="font-orbitron text-[10px] text-slate-400 uppercase tracking-widest mb-4">Predictive Threat Projection</div>
              <div className="flex gap-4 items-center">
                <div className="w-40 h-40 relative">
                  <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
                    <circle cx="50" cy="50" r="48" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="1" />
                    <circle cx="50" cy="50" r="32" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="1" />
                    <circle cx="50" cy="50" r="16" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="1" />
                    {report && (
                      <>
                        <path
                          d={`M 50 50 L ${50 + escalation * 0.4} ${50 - escalation * 0.3} L ${50 + escalation * 0.45} ${50 - escalation * 0.45}`}
                          fill="none" stroke="#ef4444" strokeWidth="1.5"
                        />
                        <circle cx={50 + escalation * 0.45} cy={50 - escalation * 0.45} r="2" fill="#ef4444" className="animate-ping" />
                      </>
                    )}
                  </svg>
                </div>
                <div className="flex-1 flex flex-col justify-center">
                  <div className="text-[9px] font-mono text-slate-500 uppercase mb-1">Probability of Escalation</div>
                  <div className={`text-4xl font-mono font-bold ${threatColor}`}>{escalation}%</div>
                  <div className="mt-4 space-y-2">
                    {projections.map(p => (
                      <div key={p.label} className="flex items-center justify-between">
                        <span className="text-[9px] font-mono text-slate-500 uppercase tracking-tighter">{p.label}</span>
                        <span className="text-xs font-mono font-bold text-red-400">{p.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
              </div>
              {report && (
                <div className="mt-6 flex items-center gap-3 bg-red-500/5 border border-red-500/20 px-3 py-2 rounded">
                  <AlertTriangle size={14} className="text-red-500" />
                  <span className="font-orbitron text-[10px] text-red-500 uppercase tracking-widest">
                    Trajectory: {escalation >= 70 ? "Escalating" : escalation >= 40 ? "Elevated" : "Stable"}
                  </span>
                </div>
              )}
            </div>
          </div>

          {/* ─── Right Column ────────────────────────────────────── */}
          <div className="flex flex-col gap-6">
            {/* Anomaly Reasons — from real contributing_factors */}
            <div className="bg-black/40 rounded-xl border border-white/5 p-5 flex flex-col flex-1">
              <div className="font-orbitron text-[10px] text-slate-400 uppercase tracking-widest mb-4">Anomaly Reasons</div>
              {report && (
                <div className="flex items-center gap-3 text-red-500 bg-red-500/10 p-3 border border-red-500/20 mb-6">
                  <AlertTriangle size={16} />
                  <span className="font-orbitron text-[11px] font-bold uppercase tracking-widest">{report.overall_threat_level} Anomaly</span>
                </div>
              )}
              <p className="text-[11px] text-slate-400 mb-6 leading-relaxed">
                {report?.behavioral_profile?.baseline_comparison ?? "Awaiting behavioral pattern analysis..."}
              </p>
              <div className="space-y-4">
                {factors.length === 0 && (
                  <div className="text-[10px] font-mono text-slate-600 uppercase italic">No contributing factors identified</div>
                )}
                {factors.slice(0, 5).map((f, i) => {
                  const Icon = REASON_ICONS[i % REASON_ICONS.length];
                  return (
                    <div key={`${f.factor}-${i}`} className="flex items-start gap-4 p-2 hover:bg-white/[0.02] transition rounded group">
                      <div className="mt-1 w-10 h-10 shrink-0 rounded-lg border border-white/5 bg-white/[0.03] flex items-center justify-center text-red-500 group-hover:border-red-500/30 transition">
                        <Icon size={20} strokeWidth={1.5} />
                      </div>
                      <div className="flex flex-col">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-orbitron text-[10px] font-bold text-slate-300 uppercase">{f.factor}</span>
                          <span className="text-[10px] font-mono text-red-500">Impact: {f.weight}%</span>
                        </div>
                        <p className="text-[9px] text-slate-500 leading-relaxed">{f.explanation}</p>
                      </div>
                    </div>
                  );
                })}
              </div>
            </div>

            {/* Behavior Baseline Comparison */}
            <div className="bg-black/40 rounded-xl border border-white/5 p-5 h-[340px]">
              <div className="font-orbitron text-[10px] text-slate-400 uppercase tracking-widest mb-6">Behavior Baseline Comparison</div>
              <div className="grid grid-cols-3 gap-2 mb-6">
                <StatBox label="Deviation Score" value={report ? `+${Math.round(report.behavioral_profile.deviation_score)}%` : "—"} color={report ? "text-red-500" : "text-slate-600"} />
                <StatBox label="Pattern Shift" value={report?.behavioral_profile.pattern_shift ?? "—"} color={report ? "text-red-500" : "text-slate-600"} />
                <StatBox label="Confidence" value={anomalies[0] ? `${Math.round(anomalies[0].confidence)}%` : "—"} color={report ? "text-teal-400" : "text-slate-600"} />
              </div>
              <div className="h-28 relative">
                <div className="absolute top-0 right-0 flex items-center gap-4 text-[8px] font-mono text-slate-500 mb-2">
                  <div className="flex items-center gap-1"><div className="w-2 h-[1px] bg-teal-500" /> Baseline</div>
                  <div className="flex items-center gap-1"><div className="w-2 h-[1px] bg-white" /> Current</div>
                </div>
                <svg className="w-full h-full overflow-visible pt-6">
                  {baselineData.length > 0 && (
                    <>
                      <path
                        d={`M ${baselineData.map(d => `${d.x * 8},${60 - (d.baseline * 0.5)}`).join(' L ')}`}
                        fill="none" stroke="#14B8A6" strokeWidth="1" strokeOpacity="0.6"
                      />
                      <path
                        d={`M ${baselineData.map(d => `${d.x * 8},${60 - (d.current * 0.5)}`).join(' L ')}`}
                        fill="none" stroke="#F8FAFC" strokeWidth="1.5"
                      />
                      <rect x="200" y="0" width="40" height="100%" fill="rgba(239,68,68,0.1)" />
                    </>
                  )}
                </svg>
                {report && <div className="absolute bottom-0 right-0 font-orbitron text-[8px] text-red-500 uppercase tracking-widest">Anomaly Zone</div>}
              </div>
            </div>
          </div>

        </div>
      </div>
    </div>
  );
}

function FeedItem({ time, label, sub, severity }: { time: string; label: string; sub: string; severity: string }) {
  const sevColor = severity === "SEVERE" ? "text-red-500" : severity === "MODERATE" ? "text-amber-500" : severity === "HIGH" ? "text-red-400" : "text-teal-400";
  return (
    <div className="flex items-center justify-between group cursor-pointer hover:bg-white/[0.02] p-1 rounded transition">
      <div className="flex items-center gap-4">
        <div className="text-[9px] font-mono text-slate-500">{time}</div>
        <div className="flex flex-col">
          <div className="text-[10px] font-bold text-slate-200">{label}</div>
          <div className="text-[8px] text-slate-500 max-w-[280px] truncate">{sub}</div>
        </div>
      </div>
      <div className={`text-[8px] font-orbitron font-bold ${sevColor} tracking-widest`}>{severity}</div>
    </div>
  );
}

function StatBox({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="bg-black/30 border border-white/5 p-2 text-center">
      <div className="text-[8px] font-orbitron text-slate-500 uppercase tracking-tighter mb-1">{label}</div>
      <div className={`text-sm font-mono font-bold ${color}`}>{value}</div>
    </div>
  );
}
