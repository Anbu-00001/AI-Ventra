...
export default function DigitalTraceMapView() {
  ...
  const tracePoints = useMemo(() => gpsData ? gpsToTracePoints(gpsData) : [], [gpsData]);
  const routeStats = useMemo(() => {
    if (!gpsData?.pings?.length) return { dist: "0.0", elapsed: "0H 00M", avg: "0", max: "0" };
    const pings = gpsData.pings;
    const speeds = pings.map(p => p.speed_kmh);
    const avg = Math.round(speeds.reduce((a, b) => a + b, 0) / Math.max(speeds.length, 1));
      ...
      <div className="flex-1 p-6 grid grid-cols-[300px_1fr_300px] gap-6 overflow-hidden">
              ...
              <div className="flex items-center gap-6">
                 <div className="relative w-24 h-24">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                       <span className="text-xl font-mono font-bold text-white">{gpsData ? "72%" : "0%"}</span>
                       <span className="text-[6px] font-orbitron text-slate-500 uppercase">{gpsData ? "High Activity" : "No Activity"}</span>
                    </div>
                 </div>
                 <div className="flex-1 space-y-2">
                    <ZoneStat label="Urban Zone" value={gpsData ? "72%" : "0%"} color="bg-amber-500" />
                    <ZoneStat label="Semi-Urban" value={gpsData ? "18%" : "0%"} color="bg-slate-500" />
                    <ZoneStat label="Industrial" value={gpsData ? "7%" : "0%"} color="bg-slate-600" />
                    <ZoneStat label="Rural" value={gpsData ? "3%" : "0%"} color="bg-slate-700" />
                 </div>
              </div>
           </div>
        </div>
          ...
          <div className="flex-1 relative overflow-hidden">
            {!gpsData && (
              <div className="absolute inset-0 z-50 flex items-center justify-center p-12">
                <EmptyState message="No GPS or digital trace logs available. Geo-intelligence requires device metadata ingestion." />
              </div>
            )}
            <div className={`bg-[#070b14] rounded-xl border border-white/10 h-full relative overflow-hidden group ${!gpsData ? 'opacity-20 pointer-events-none grayscale' : ''}`}>
           ...
           <div className="grid grid-cols-2 gap-6 h-48">
              ...
              <div className="bg-black/40 rounded-xl border border-white/5 p-5">
                 ...
                 <div className="flex flex-col gap-2">
                    <div className="text-[8px] font-mono text-slate-500 uppercase tracking-widest">{gpsData ? `Device: ${gpsData.device_id} | Owner: ${gpsData.owner}` : "No Active Device"}</div>
                    <div className="flex items-center gap-2 font-mono text-[10px] text-slate-300">
                       <span>{gpsData ? `${gpsData.total_pings} pings` : "0 pings"}</span> <div className="w-2 h-px bg-slate-700" /> 
              ...
              <div className="bg-black/40 rounded-xl border border-white/5 p-5">
                 ...
                 <div className="space-y-2 max-h-24 overflow-y-auto custom-scrollbar pr-2">
                    {gpsData ? (
                      <>
                        <LocationRow name="Origin Detected" time="02:14 AM" />
                        <LocationRow name="Outer Ring Road" time="02:27 AM" />
                        <LocationRow name="CCTV Point 7" time="02:47 AM" />
                        <LocationRow name="Phoenix Mall" time="03:18 AM" />
                      </>
                    ) : (
                      <div className="text-[10px] font-mono text-slate-600 uppercase">Waiting for data ingestion...</div>
                    )}
                 </div>
              </div>
           </div>
        ...
        <div className="flex flex-col gap-6 overflow-y-auto custom-scrollbar">
           ...
           <div className="bg-black/40 rounded-xl border border-white/5 p-5 flex-1 flex flex-col">
              ...
              <div className="space-y-6 relative flex-1">
                 ...
                 {gpsData ? (
                   ...
                   <div className="text-[10px] font-mono text-slate-600 uppercase pl-8 italic">No events recorded.</div>
                 )}
              </div>
           </div>
           ...
           <div className="bg-black/40 rounded-xl border border-white/5 p-5 h-[340px] flex flex-col">
              <div className="font-orbitron text-[10px] text-slate-400 uppercase tracking-widest mb-6">Signal Strength Analysis</div>
              
              <div className="h-24 mb-8">
                 <svg viewBox="0 0 200 60" className="w-full h-full">
                    <path 
                      d={gpsData ? "M 0 30 Q 20 10, 40 40 T 80 20 T 120 50 T 160 30 T 200 20" : "M 0 30 L 200 30"}
                      fill="none" stroke={gpsData ? "#14B8A6" : "#334155"} strokeWidth="1.5" 
                    />
                 </svg>
              ...
              <div className="font-orbitron text-[10px] text-slate-400 uppercase tracking-widest mb-4">Signal Integrity</div>
              <div className="flex gap-4">
                 <div className="w-20 h-20 relative">
                    <div className="absolute inset-0 border border-white/5 rounded-full" />
                    <svg viewBox="0 0 100 100" className="w-full h-full">
                       <path d="M 50 0 L 50 100 M 0 50 L 100 50" stroke="rgba(255,255,255,0.05)" />
                       {gpsData && <circle cx="50" cy="50" r="4" fill="#14B8A6" />}
                    </svg>
                 </div>
                 <div className="flex-1 flex flex-col justify-center">
                    <div className={`font-bold text-xs uppercase mb-1 ${gpsData ? 'text-teal-400' : 'text-slate-600'}`}>{gpsData ? 'Signal Locked' : 'No Signal'}</div>
                    <ul className="text-[9px] text-slate-500 space-y-1">
                       <li>• Network: {gpsData ? 'Active' : 'Idle'}</li>
                       <li>• Precision: {gpsData ? 'High' : 'N/A'}</li>
                    </ul>
                 </div>
              </div>
              <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-4">
                 <span className="text-[9px] font-orbitron text-slate-500 uppercase">Confidence</span>
                 <span className={`text-sm font-mono font-bold ${gpsData ? 'text-teal-400' : 'text-slate-700'}`}>{gpsData ? '94%' : '0%'}</span>
              </div>
           </div>
        </div>
      </div>
