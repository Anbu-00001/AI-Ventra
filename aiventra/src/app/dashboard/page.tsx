...
// No hardcoded evidence files — loaded from backend on mount

const defaultParsingSteps = [
  {
    label: "Decrypting Evidence",
    sub: "Waiting for evidence upload",
    value: 0,
    state: "Pending",
    color: "teal",
    icon: Lock,
...
// Classifiers are now dynamically derived from uploaded files

const railItems = [
  { icon: ScanSearch, label: "Intake Terminal", view: "intake" },
  { icon: FolderUp, label: "Evidence Upload", view: "intake" },
  { icon: Camera, label: "Visual Intelligence", view: "camera" },
...
const VisualIntelligenceView = dynamic(
  () => import("@/components/dashboard/VisualIntelligenceView"),
  { ssr: false }
);
...
type DashboardView = "intake" | "autopsy" | "correlation" | "timeline" | "anomaly" | "triage" | "camera";
type IntakeFile = {
  title: string;
  type: string;
...
export default function DashboardPage() {
  ...
  const [steps, setSteps] = useState<ParsingStep[]>(defaultParsingSteps);
  ...
  const classifiers = useMemo(() => {
    if (!files.length) {
      return [
        { label: "Physical Evidence", confidence: 0, color: "crimson", icon: Fingerprint },
        { label: "Digital Evidence", confidence: 0, color: "teal", icon: Cpu },
        { label: "Behavioral Evidence", confidence: 0, color: "amber", icon: Brain },
        { label: "Environmental Data", confidence: 0, color: "green", icon: Leaf },
      ];
    }
    return [
      { label: "Physical Evidence", confidence: files.some(f => f.type === "PDF" || f.type === "TXT") ? 94 : 22, color: "crimson", icon: Fingerprint },
      { label: "Digital Evidence", confidence: files.some(f => f.type === "JSON" || f.type === "LOG" || f.type === "CSV") ? 98 : 15, color: "teal", icon: Cpu },
      { label: "Behavioral Evidence", confidence: files.some(f => f.type === "JSON") ? 87 : 45, color: "amber", icon: Brain },
      { label: "Environmental Data", confidence: files.some(f => f.type === "CSV") ? 91 : 10, color: "green", icon: Leaf },
    ];
  }, [files]);
  ...
  // ── Load backend health + RAG stats + previously uploaded files ─────────
  useEffect(() => {
    let mounted = true;
    ...
    const refreshBackend = async () => {
      try {
        ...
        setStatusLine(`AI ${h.llm} | RAG ${stats.data.status} | ${stats.data.total_vectors} vectors`);
      } catch (error) {
        if (!mounted) return;
    ...
    const loadExistingFiles = async () => {
      try {
        ...
        setFiles(loaded);
        // Mark parsing steps as complete since files are already processed
        setSteps(current => current.map(s => ({
          ...s,
          value: 100,
          state: "Complete",
          sub: `${res.data.length} evidence files processed`,
        })));
        setStatusLine(`${res.data.length} evidence files loaded · RAG indexed`);
      } catch {
        // No previously uploaded files — that's fine
      }
    };
  ...
  const handleFiles = async (selected: FileList | null) => {
    ...
    try {
      ...
      setStatusLine("Upload failed - check backend connection");
      alert("Evidence upload failed. Ensure the backend is reachable.");
    } finally {
      setIsUploading(false);
    }
  };
          ...
          <div className="relative flex min-w-0 flex-1 flex-col">
            ...
            <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 p-4 lg:grid-cols-[minmax(0,1.72fr)_minmax(340px,0.82fr)] lg:p-6">
              <section className="relative min-h-[520px] overflow-hidden border border-white/8 bg-[#050910]/82">
                  <div className="absolute left-5 top-5 z-30 flex w-[calc(100%-40px)] items-center justify-between">
                    <div>
                    ...
                    <button
                      type="button"
                      onClick={async () => {
                        if (confirm("Permanently wipe all evidence, findings, and vector indices? This cannot be undone.")) {
                          try {
                            await wipeAllData();
                            setFiles([]);
                      ...
                      className="flex items-center gap-2 border border-white/10 bg-white/5 px-4 py-2 font-orbitron text-[10px] uppercase tracking-widest text-slate-400 transition hover:border-crimson/50 hover:bg-crimson/5 hover:text-crimson-glow"
                    >
                      <Trash2 size={14} />
                      Wipe Evidence
                    </button>
              ...
              <aside className="grid min-h-0 gap-4">
                ...
                <Panel title="Visual Intelligence" subtitle="Neural CCTV Correlation">
                  <div className="space-y-4">
                    <div className="relative h-24 w-full overflow-hidden border border-white/10 bg-black/40">
                      <div className="absolute inset-0 opacity-20 dashboard-noise" />
                      <div className="absolute left-2 top-2 font-mono text-[8px] text-teal-data uppercase tracking-widest">
                        CAM_LIVE_01
                      </div>
                      ...
                      <div className="absolute bottom-2 left-2 flex items-center gap-1">
                        ...
                        <span className="font-mono text-[8px] text-slate-400">TRACKING ID: 8219-X</span>
                      </div>
                    </div>
                    <div className="p-3 bg-white/[0.03] border border-white/5 rounded">
                      <div className="font-orbitron text-[9px] text-teal-data uppercase tracking-widest mb-1">
                        Recent Detection
                      </div>
                      <div className="font-mono text-[10px] text-slate-300">
                        Object matched against suspect baseline (94% confidence)
                      </div>
                    </div>
                  </div>
                </Panel>
